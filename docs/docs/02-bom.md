...
| Wireless charge Rx | Qi receiver module, 5 V output, ~50 mm coil | PURCHASED — measure its real delivered current |
...
| Charge feed | USB-C male plug breakout or short pigtail, CC pins broken out | **NEEDED** (D-010) |
| CC pull-ups | 2 × 56 kΩ resistors per pad | **NEEDED** (D-010) |
...
| Role | Part | Status |
|---|---|---|
| Qi transmitter pad | One per charging position | PURCHASED |
...
- Two modules are now surplus per pad: the MT3608 (D-002) and the TP4056
  (D-010). Do not re-add either without reading the decision log.
